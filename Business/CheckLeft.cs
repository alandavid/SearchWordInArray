...
namespace Business
{
    public class CheckLeft : IDirection
    {
        private IDirection next;

        public CheckLeft(IDirection direccion)
        {
            this.next = direccion;
        }
        public int[,] checkDireccion(char[,] matriz, string word, int row, int col)
        {
            ...
            if (col - (word.Count() - 1) >= 0)
            {
                ...
                {
                    if (matriz[row, col - offset] != word.ElementAt(offset))
                    {
        ...
        public void ConvertToResponseExamen(ref int[,] array, int rowResponse, int colResponse, int offSetResponse)
        {
            ...
            array[offSetResponse, 1] = (offSetResponse - 1) + 1;
